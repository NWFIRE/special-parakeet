...
export default async function InspectionReportDetailPage({ params }: { params: Promise<{ reportId: string }> }) {
  ...
  const customerSignature = ((report.customerSignature as SignatureShape | null) ?? null);
  const technicianSignature = ((report.technicianSignature as SignatureShape | null) ?? null);
      ...
      <Card className="glass-panel-strong overflow-hidden p-0">
        <div className="flex flex-col gap-6 px-6 py-6 lg:flex-row lg:items-start lg:justify-between lg:px-8">
          <div>
            ...
            <p className="mt-3 text-sm leading-7 text-slate-600">{report.client.companyName} | {report.site?.name ?? report.propertyName}</p>
          </div>
