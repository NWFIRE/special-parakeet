...
function buildReportNumber(serviceType: InspectionServiceType) {
  const prefixMap: Record<InspectionServiceType, string> = { FIRE_EXTINGUISHER: "FE", FIRE_ALARM: "FA", FIRE_SPRINKLER: "FS", KITCHEN_SUPPRESSION: "KS", EMERGENCY_EXIT_LIGHTING: "EL", BACKFLOW: "BF", OTHER: "IR" };
...
function buildLegacyDeficiencies(assets: Array<{ assetName: string; location?: string; status: InspectionOutcome; deficiencySummary: string; recommendationText: string; nextServiceDate?: string; followUpRequired: boolean }>) {
  return assets.filter((asset) => asset.status !== "PASS" || asset.followUpRequired || asset.deficiencySummary || asset.recommendationText).map((asset) => ({ description: asset.deficiencySummary || `${asset.assetName} requires follow-up service.`, severity: asset.status, location: asset.location ?? "", corrective_action: asset.recommendationText || "Review and complete corrective work.", due_date: asset.nextServiceDate || "" }));
}
...
function buildEquipmentSummary(assets: Array<{ assetName: string; manufacturer?: string; location?: string; status: InspectionOutcome; deviceType?: string }>) {
  return assets.map((asset) => ({ category: asset.deviceType || asset.assetName, quantity: "1", notes: [asset.manufacturer, asset.location, asset.status].filter(Boolean).join(" Ãƒâ€šÃ‚Â· ") }));
}

function serializeSignature(name: string | undefined) {
  return name ? { name, signedAt: new Date().toISOString() } : null;
}
...
function valueAtPath(asset: { [key: string]: unknown; attributes: Record<string, string>; checks: Array<{ key: string; status: InspectionOutcome; note: string }> }, path: string) {
  ...
  if (path.startsWith("checks.")) { const [, key, field] = path.split("."); const check = asset.checks.find((item) => item.key === key); return !check ? "" : field === "note" ? check.note : check.status; }
...
async function saveInspectionReport(rawPayload: string, expectedReportId?: string) {
  ...
  const [client, site] = await Promise.all([ensureClientBelongsToTeam(payload.clientId, membership.teamId), ensureSiteBelongsToTeam(payload.siteId, membership.teamId, payload.clientId)]);
  ...
  const reportData = {
    teamId: membership.teamId, clientId: payload.clientId, siteId: payload.siteId, title: payload.title, reportNumber: payload.reportNumber || buildReportNumber(payload.serviceType), inspectionType: toLegacyInspectionType(payload.serviceType), serviceType: payload.serviceType, status: payload.status, overallStatus, propertyName: payload.propertyName, propertyAddress: payload.propertyAddress || null, inspectorName: payload.inspectorName || user.name || null, pointOfContact: payload.pointOfContact || null, frequencyLabel: payload.frequencyLabel || serviceConfig.defaultFrequency, serviceDate: parseOptionalDate(payload.serviceDate) ?? new Date(), completedAt: parseOptionalDate(payload.completedAt), nextInspectionDate: parseOptionalDate(payload.nextInspectionDate), summary: payload.summary || null, recommendationSummary: payload.recommendations || null, notes: payload.notes || null, codeReferences: payload.codeReferences, checklistItems: buildLegacyChecklist(payload.assets), deficiencies: buildLegacyDeficiencies(payload.assets), equipmentSummary: buildEquipmentSummary(payload.assets), technicianLicense: payload.technicianLicense || null, technicianCertification: payload.technicianCertification || null, customerSignature: serializeSignature(payload.customerSignatureName || undefined) ?? Prisma.JsonNull, technicianSignature: serializeSignature(payload.technicianSignatureName || undefined) ?? Prisma.JsonNull, photoUrls: payload.photoUrls, autoFillSummary: payload.autoFillSummary || stringifyAutofillSummary(payload.assets), finalizedAt, portalPublishedAt: payload.status === "DRAFT" ? null : new Date()
  };

  const report = await db.$transaction(async (tx) => {
    const savedReport = payload.reportId ? await tx.inspectionReport.update({ where: { id: payload.reportId }, data: reportData }) : await tx.inspectionReport.create({ data: reportData });
    ...
    for (let index = 0; index < payload.assets.length; index += 1) {
      ...
      const assetData = { clientId: payload.clientId, siteId: payload.siteId, serviceType: payload.serviceType, name: assetDraft.assetName, location: assetDraft.location || null, assetTag: assetDraft.assetTag || null, deviceType: assetDraft.deviceType || null, manufacturer: assetDraft.manufacturer || null, model: assetDraft.model || null, serialNumber: assetDraft.serialNumber || null, ulListing: assetDraft.ulListing || null, complianceFrequency: assetDraft.complianceFrequency || serviceConfig.defaultFrequency, lastServiceDate: parseOptionalDate(assetDraft.lastServiceDate), nextServiceDate: parseOptionalDate(assetDraft.nextServiceDate), profileData: assetDraft.attributes, lastInspectionData: { status: assetDraft.status, deficiencySummary: assetDraft.deficiencySummary, recommendationText: assetDraft.recommendationText, followUpRequired: assetDraft.followUpRequired, deficiencyTemplateKey: assetDraft.deficiencyTemplateKey, checks: assetDraft.checks, attributes: assetDraft.attributes, lastServiceDate: assetDraft.lastServiceDate, nextServiceDate: assetDraft.nextServiceDate, reportNumber: reportData.reportNumber, savedAt: new Date().toISOString() } };
      const assetRecord = existingAsset ? await tx.inspectionAsset.update({ where: { id: existingAsset.id }, data: assetData }) : await tx.inspectionAsset.create({ data: { teamId: membership.teamId, ...assetData } });
      const snapshot = await tx.inspectionReportAsset.create({ data: { reportId: savedReport.id, assetId: assetRecord.id, sortOrder: index, assetName: assetDraft.assetName, location: assetDraft.location || null, assetTag: assetDraft.assetTag || null, deviceType: assetDraft.deviceType || null, manufacturer: assetDraft.manufacturer || null, model: assetDraft.model || null, serialNumber: assetDraft.serialNumber || null, ulListing: assetDraft.ulListing || null, complianceFrequency: assetDraft.complianceFrequency || null, lastServiceDate: parseOptionalDate(assetDraft.lastServiceDate), nextServiceDate: parseOptionalDate(assetDraft.nextServiceDate), status: assetDraft.status, deficiencySummary: assetDraft.deficiencySummary || null, recommendationText: assetDraft.recommendationText || null, followUpRequired: assetDraft.followUpRequired, attributes: assetDraft.attributes, testResults: assetDraft.checks, codeReferences: payload.codeReferences, appliedTemplateKeys: assetDraft.deficiencyTemplateKey ? [assetDraft.deficiencyTemplateKey] : [], autofillMeta: assetDraft.autofillMeta } });
      snapshots.push({ id: snapshot.id, payloadIndex: index });
    }

    for (const snapshot of snapshots) {
      const assetDraft = payload.assets[snapshot.payloadIndex];
      const auditRows = Object.entries(assetDraft.autofillMeta).map(([fieldPath, meta]) => ({ reportId: savedReport.id, reportAssetId: snapshot.id, changedById: user.id, fieldPath, sourceType: meta.sourceType, sourceLabel: meta.sourceLabel, sourceValue: meta.sourceValue, finalValue: String(valueAtPath(assetDraft, fieldPath) ?? ""), wasOverridden: String(valueAtPath(assetDraft, fieldPath) ?? "") !== meta.sourceValue }));
...
export async function createSiteAction(_: typeof initialState | undefined, formData: FormData) {
  try {
    ...
    await db.clientSite.create({ data: { teamId: membership.teamId, clientId: parsed.data.clientId, name: parsed.data.name, siteCode: parsed.data.siteCode || null, addressLine1: parsed.data.addressLine1, addressLine2: parsed.data.addressLine2 || null, city: parsed.data.city || null, state: parsed.data.state || null, postalCode: parsed.data.postalCode || null, contactName: parsed.data.contactName || null, contactEmail: parsed.data.contactEmail || null, contactPhone: parsed.data.contactPhone || null, notes: parsed.data.notes || null } });
